---
name: steering-manual-web-test
description: Manual web test procedure. Activate when running manual UI/browser tests.
---
...
3. **Deploy artifacts** (nếu cần) — copy output vào vị trí chạy:
   - dist/ hoặc build/ → copy vào thư mục deploy
...
4. **Detect start command** — xác định cách start server:
   ...
   - Node.js compiled bundle → `node dist/index.js`
...
- `execute_pwsh` — build, copy artifacts
