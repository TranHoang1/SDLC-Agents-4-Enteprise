...
beforeEach(() => {
  ...
  insert.run(1, 'c1', 'a', 'note', 'PROJECT', null, 'p1', '2024-01-01 00:00:00');
  insert.run(2, 'c2', 'b', 'note', 'PROJECT', null, 'p1', '2024-01-02 00:00:00');
  insert.run(3, 'c3', 'c', 'note', 'USER', 'u1', 'p1', '2024-01-03 00:00:00');
  insert.run(4, 'c4', 'd', 'note', 'PROJECT', null, 'other', '2024-01-04 00:00:00');
  insert.run(5, 'c5', 'e', 'note', 'SHARED', null, null, '2024-01-05 00:00:00');
  db.prepare("INSERT INTO kb_shared_grants (project_id, granted_by) VALUES ('p1', 'u1')").run();
  repo = new KbRepository(makeTestAdapter(db));
});
...
describe('KbRepository', () => {
  ...
  it('getEntries falls back to created_at for invalid sort columns', () => {
    ...
    expect(result.items.map((r) => r.id)).toEqual([1, 2, 3, 4, 5]);
    expect(db.prepare("SELECT name FROM sqlite_master WHERE name = 'knowledge_entries'").get()).toBeDefined();
  });
