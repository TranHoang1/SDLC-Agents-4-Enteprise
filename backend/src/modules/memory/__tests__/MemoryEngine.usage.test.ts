...
describe('MemoryEngine tool usage', () => {
  ...
  it('UT-06: first call inserts row with call_count = 1', () => {
  ...
  it('UT-07: subsequent calls increment existing row (UPSERT, single row)', () => {
    for (let i = 0; i < 3; i++) ctx.engine.incrementToolUsage('code_search');
    const rows = ctx.engine.getToolUsage('code_search');
    expect(rows).toHaveLength(1);
    expect(rows[0].call_count).toBe(3);
  });

  it('UT-08: no filter returns rows ordered by call_count DESC', () => {
    ...
    const rows = ctx.engine.getToolUsage();
    expect(rows.map(r => r.tool_name)).toEqual(['find_tools', 'mem_search', 'code_search']);
    expect(rows.map(r => r.call_count)).toEqual([9, 5, 2]);
  });

  it('UT-09: filter by tool_name returns single row; unknown returns empty', () => {
    ctx.engine.incrementToolUsage('mem_search');
    ctx.engine.incrementToolUsage('code_search');
    const one = ctx.engine.getToolUsage('mem_search');
    expect(one).toHaveLength(1);
    expect(one[0].tool_name).toBe('mem_search');
    expect(ctx.engine.getToolUsage('never_called')).toEqual([]);
  });
});
