/**
 * SA4E — Unit tests (part 2) for the workspace file scanner: scanWorkspace
 * traversal/exclusion behaviour and loadFileMetadata sidecar handling.
 * Split from file-scanner.test.ts to keep each file under the CI line-count
 * gate (≤200 lines). No logic changed — tests only relocated.
 */
...
describe('scanWorkspace', () => {
  ...
  // ---- SA4E-223: Gate 2 sync — new extensions pass when in includeExtensions (TC-04a) ----
  it('passes Gate 2 for new Salesforce extensions when configured', () => {
    const cfg = config({ includeExtensions: ['.page', '.apex', '.cmp', '.component', '.tokens', '.app', '.evt', '.intf', '.soql'] });
    write('ui/MyPage.page', '<apex:page controller="Foo"/>');
    write('code/MyClass.apex', 'public class MyClass {}');
    write('aura/MyCmp.cmp', '<aura:component/>');

    const results = scanWorkspace(cfg);
    const langs = results.map(r => `${r.relativePath}:${r.language}`).sort();
    expect(langs).toEqual([
      'aura/MyCmp.cmp:aura',
      'code/MyClass.apex:apex',
      'ui/MyPage.page:visualforce',
    ]);
  });

  it('skips new Salesforce extensions at Gate 2 when not in includeExtensions', () => {
    const results = scanWorkspace(config()); // baseConfig lacks .page/.apex/...
    expect(results.map(r => r.relativePath).filter(p => p.endsWith('.page') || p.endsWith('.apex'))).toEqual([]);
  });
});
