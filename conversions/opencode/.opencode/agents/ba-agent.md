...
1. Call `embed_images` to create self-contained markdown:
...
2. Call `export_docx` with **file_path**:
...
- **KB Ingest**: After creating/updating BRD or FSD, ingest FULL content into KB via mem_ingest(type="document", content="<full content>", tags=["brd", "{TICKET}"])
