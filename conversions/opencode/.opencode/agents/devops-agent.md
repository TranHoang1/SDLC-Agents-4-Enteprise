...
- **KB Ingest**: After creating DPG/RLN, ingest FULL content into KB: mem_ingest(type="document", content="<full content>", tags=["dpg", "rln", "{TICKET}"])
- **Version Sync**: After creating git tag, verify README.md badge versions match tagged version
- **Drawio Validate**: After creating deployment/rollback diagrams, validate XML
- **KB Search First**: Before creating DPG, search KB for existing deployment docs: mem_search(query: "{TICKET} deployment")
