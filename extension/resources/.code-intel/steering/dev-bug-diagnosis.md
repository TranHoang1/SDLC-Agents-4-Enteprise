...
2. Verify existing tests run: `npm test`
...
describe('Provider creation validation', () => {
  it('should reject empty provider name', async () => {
    ...
    const request = { name: "", transport: "stdio" };
...
// only checks for null. Line 42 of validation-service.ts.
```
...
2. Ensure the reproduction test is properly named and documented:
   ```typescript
   describe('BUG-{TICKET}: should reject empty provider name', () => { ... });
   ```
