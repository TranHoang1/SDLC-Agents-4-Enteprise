...
description: 'find_tools → execute_dynamic_tool pattern for nested MCP tools'
applyTo: '**'
---
...
MCP server exposes tools from child servers via orchestration. These nested tools are NOT directly callable — they return "Unknown tool" if called directly.
...
|---|---|---|
| Jira | `find_tools("jira")` | jira_get_issue, jira_search |
| Export | `find_tools("export docx")` | export_docx, embed_images |
| Draw.io | `find_tools("drawio")` | drawio_auto_layout, drawio_export_png |

## Rules

1. NEVER call nested tools directly (returns "Unknown tool")
2. ALWAYS use `execute_dynamic_tool` as execution wrapper
3. Arguments must be objects — `{"issue_key": "X"}` not `"{\"issue_key\": \"X\"}"`
4. Core tools (`mem_search`, `mem_ingest`, `find_tools`, `code_search`, `agent_log`) are directly callable

## Error Recovery

- Schema validation error → check argument types against inputSchema
- Tool not found → child server may be DEAD, check `orchestration_status`
