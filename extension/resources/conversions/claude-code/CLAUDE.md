...
SM reads `.claude/rules/loop-constraints.md` before Step 0. See that file for hard guardrails.

### Domain Glossary Extraction (Phase 1 — after BRD)

BA MUST extract ≥5 domain terms into KB after BRD creation:
```
...
### Two-Axis Code Review (Phase 6 — before QA tests)

SM runs 2 PARALLEL reviews:
- **Axis 1 (Standards):** DEV agent reviews code standards, SOLID, Fowler smells
- **Axis 2 (Spec Compliance):** QA agent checks TDD/FSD compliance, missing features, scope creep

Both PASS → proceed to QA tests. Either FAIL → DEV fixes (max 2 iterations).
...
- Two-Axis Code Review BEFORE QA tests (Standards + Spec Compliance, parallel)
