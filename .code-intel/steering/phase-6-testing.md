...
invokeSubAgent(
  name: "qa-agent",
  prompt: "Chạy automated tests cho {TICKET}. Run npm test. Báo cáo pass/fail."
)
```
...
2. Read actual IT test source files (`*integration.test.ts`)
...
|----------|---------|--------|
| IT mocks ALL deps (vi.fn everywhere) | Not real integration test | ❌ Send back to DEV |
| IT calls service directly (no HTTP) | Missing API layer testing | ❌ Send back to DEV |
| IT has no real DB when STC requires | Missing real DB/infra | ❌ Send back to DEV |
