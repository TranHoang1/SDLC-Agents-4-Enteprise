...
## 6. Kiến trúc mục tiêu SA4E-85 (Cập nhật)

### 6.1. Backend-Driven Knowledge Architecture

Quyết định kiến trúc mới của dự án:

- **Không lưu conversation state ở Client (Webview hoặc Extension Host).**
- **Knowledge Base (KB) trên Backend là Single Source of Truth.**
- LangGraph Runtime chỉ thực thi workflow và đọc/ghi checkpoint thông qua Knowledge Service.
- Mọi IDE (VSCode, Kiro, AntiGravity...) đều hydrate từ Backend theo `thread_id`.

Kiến trúc:

```text
Svelte Webview (Stateless)
        │
Extension Host (Thin Proxy)
        │
...
Không sử dụng SQLite Checkpointer cục bộ làm nguồn dữ liệu chính.

### 6.3. Bổ sung Roadmap

| Ưu tiên | Hạng mục |
|---|---|
| P0 | Backend Knowledge Service |
| P0 | KB Checkpointer thay cho Client State |
| P0 | Stateless Extension Host |
| P0 | Stateless Svelte Webview |
| P1 | Multi-IDE Hydration từ KB |
| P1 | Event Sourcing |
| P1 | Artifact Store |
| P2 | Capability-based Permission Model |
| P2 | Versioned IPC Protocol |

### 6.4. Nguyên tắc thiết kế

1. Backend là nguồn dữ liệu duy nhất.
2. Client chỉ hiển thị dữ liệu và gửi hành động.
3. Mọi interrupt/resume đều được lưu trong KB.
4. Không cache conversation tại client.
5. Hỗ trợ nhiều IDE kết nối đồng thời trên cùng `thread_id`.
