...
| Git Tag | `v1.36.0` (annotated, → `cb83296`) |
| Author | DevOps Agent |
| Status | Released |
...
|---------|------|--------|---------|
| 1.0 | 2026-08-24 | DevOps Agent | Initiate document for release v1.36.0 |
...
| Version-bump commit | `cb83296` — all 3 package.json: 1.35.0 → 1.36.0 (**tag target**) |
| README changelog commit | `c492837` |
| Tag hygiene | Stale tag `v1.34.0` deleted local+remote (had pointed at wrong commit `1a7a800` of SA4E-206) |
...
|----------|-----------|-------------|
| `version` ×3 package.json | Modified | 1.35.0 → **1.36.0** (commit `cb83296`) |
...
Fully compatible: v1.36.0 only adds commands and modules; no existing command, config format, or watcher behavior changed. Files generated by v1.36.0 remain readable by older versions (standard `.code-intel` conventions). Rollback to v1.35.0 requires no user data action.
...
|------|--------|---------------|
| 1 | Checkout tag `v1.36.0` + verify lineage (`3d924ca`) | 2 min |
...
- Minor cosmetic issue with workaround → hotfix v1.36.1, no rollback

**Estimated Rollback Time:** ≤ 15 minutes (VSIX downgrade to archived v1.35.0; user workspace data unaffected).
...
*RLN v1.0 — DevOps Agent, 2026-08-24. Release v1.36.0 · merge `3d924ca` · tag `v1.36.0` → `cb83296`. Sources: BRD v2.0, TDD v2.0, TEST-REPORT v3.0.*
